...
const EICAR_SIGNATURE_FOUND_RESPONSE: &[u8] = b"stream: Eicar-Signature FOUND\0";
...
fn scan_socket_infected_file() {
    ...
    let response =
        clamav_client::scan_socket(EICAR_TEST_FILE_PATH, TEST_SOCKET_PATH, None).expect(&err_msg);
    assert_eq!(&response, EICAR_SIGNATURE_FOUND_RESPONSE);
...
fn scan_socket_clean_file() {
    ...
    let response =
        clamav_client::scan_socket(CLEAN_TEST_FILE_PATH, TEST_SOCKET_PATH, None).expect(&err_msg);
...
fn scan_tcp_infected_file() {
    ...
    let response =
        clamav_client::scan_tcp(EICAR_TEST_FILE_PATH, TEST_HOST_ADDRESS, None).expect(&err_msg);
    assert_eq!(&response, EICAR_SIGNATURE_FOUND_RESPONSE);
    assert_eq!(clamav_client::clean(&response), Ok(false));
}
...
fn scan_tcp_clean_file() {
    ...
    let response =
        clamav_client::scan_tcp(CLEAN_TEST_FILE_PATH, TEST_HOST_ADDRESS, None).expect(&err_msg);
    assert_eq!(&response, OK_RESPONSE);
    assert_eq!(clamav_client::clean(&response), Ok(true));
}
